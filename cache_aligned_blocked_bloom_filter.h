...
#include "murmur3.c"
#include "cache_aligned_bit_vector.h"


class cache_aligned_blocked_bloom_filter
{
private:
    uint64_t n;     // Expected number of distinct keys to be inserted.
    double p;       // Desired nominal false positive rate.
    ...
    bit_vector B;   // The bit-vector for the blocked Bloom filter.
...
public:
    ...
    cache_aligned_blocked_bloom_filter() {}
    ...
    // last parameter.
    cache_aligned_blocked_bloom_filter(uint64_t numDistinctKeys, double falsePositiveRate, uint64_t blockSizeinByte = 0);
    ...
    // named 'outputFile'.
    cache_aligned_blocked_bloom_filter(std::string &keyFile, uint64_t numDistinct, double fpr, std::string &outputFile);
    ...
    cache_aligned_blocked_bloom_filter(std::string &bbfFile);
...
void cache_aligned_blocked_bloom_filter::dump_metadata()
{
    std::clog << "\tn = " << n << ";";
    std::clog << "\tp = " << p << ";";
    std::clog << "\tm = " << m << ";";
...
void cache_aligned_blocked_bloom_filter::set_parameters(uint64_t numDistinctKeys, double falsePositiveRate,
...
void cache_aligned_blocked_bloom_filter::set_hash_data_structure()
{
    B.set_len(m);
...
cache_aligned_blocked_bloom_filter::cache_aligned_blocked_bloom_filter(uint64_t numDistinctKeys,
...
cache_aligned_blocked_bloom_filter::cache_aligned_blocked_bloom_filter(std::string &keyFile,
                                                                        uint64_t numDistinct, double fpr,
...
cache_aligned_blocked_bloom_filter::cache_aligned_blocked_bloom_filter(std::string &bbfFile)
{
    deserialize(bbfFile);
...
uint64_t cache_aligned_blocked_bloom_filter::get_murmur_hash_block(uint64_t &key)
{
    uint64_t H[2];
...
uint64_t cache_aligned_blocked_bloom_filter::get_murmur_hash_slot(uint64_t &key, uint32_t &seed)
{
    uint64_t H[2];
...
void cache_aligned_blocked_bloom_filter::insert(uint64_t key)
{
    uint64_t block = get_murmur_hash_block(key);

    uint64_t offset = block * blockSize;
    for(uint32_t seed = 1; seed < k; ++seed)
        B.set_bit(offset + get_murmur_hash_slot(key, seed));
}



bool cache_aligned_blocked_bloom_filter::query(uint64_t key)
{
    uint64_t block = get_murmur_hash_block(key);

    uint64_t offset = block * blockSize;
    for(uint32_t seed = 1; seed < k; ++seed)
        if(!B.get_bit(offset + get_murmur_hash_slot(key, seed)))
            return false;
...
void cache_aligned_blocked_bloom_filter::query(std::string &queryFile, std::vector<bool> &result)
{
...
void cache_aligned_blocked_bloom_filter::serialize(std::string &outputFile)
{
    std::ofstream output;
...
void cache_aligned_blocked_bloom_filter::deserialize(std::string &bbfFile)
{
    std::ifstream input;
