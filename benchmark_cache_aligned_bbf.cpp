...
void benchmark_bbf_all_negative(const uint64_t elemsStart, const uint64_t elemsEnd, const uint64_t elemsStep,
                                ...
                                const uint64_t queryCount)
{
    ...
    for(double fpr = fprStart; fpr <= fprEnd; fpr += fprStep)
        for(uint64_t n = elemsStart; n <= elemsEnd; n += elemsStep)
        {
            ...
            cache_aligned_blocked_bloom_filter bbf(n, fpr);
...
void benchmark_bbf_half_negative(const uint64_t elemsStart, const uint64_t elemsEnd, const uint64_t elemsStep,
                                const double fprStart, const double fprEnd, const double fprStep,
                                const uint64_t queryCount)
{
    std::vector<uint64_t> keyList;
...
int main()
{
    const uint64_t elemsStart = 1000000;    // 1M
    const uint64_t elemsEnd = 50000000;     // 25M
    const uint64_t elemsStep = 1000000;     // 1M
    const double fprStart = 0.05;
    const double fprEnd = 0.25;
    const double fprStep = 0.05;
    const uint64_t queryCount = 1000000;    // 1M
